...
const CartPage = () => {
  ...
  const {cart,
    ...
    cartCount,
    totalCartPrice,
    removeFromCart} = useContext(AppContext);
    ...
    const placeOrder = async () => {
        try {
            ...
            const {data} = await axios.post("/order/place-order",{
                items: cart.map((item) => (
                    {
                        product: item._id,
                ...
                <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                    <div className="flex items-center md:gap-6 gap-3">
                        <div className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden">
                            <img className="max-w-full h-full object-cover" src={`${product.image}`} alt={product.title} />
                        </div>
                        <div>
        ...
        <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
            ...
            <div className="mb-6">
                ...
                <div className="relative flex justify-between items-start mt-2">
                    <p className="text-gray-500">
                        {selectedAddress ? `${selectedAddress.city},${selectedAddress.state},${selectedAddress.country}`:
                        "No Address Found"}
                    </p>
            ...
            <div className="text-gray-500 mt-4 space-y-2">
                <p className="flex justify-between">
                    <span>Total Carts</span><span>{cartCount}</span>
                </p>
                <p className="flex justify-between">
                ...
                <p className="flex justify-between text-lg font-medium mt-3">
                    <span>Total Amount:</span><span>${totalCartPrice}</span>
                </p> 
            </div>

            <button onClick={() => {paymentOption === "COD" ? placeOrder() : createOrderStripe()}} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium transition">
